...
namespace MRS.DocumentManagement.Services
{
    public class ProjectService : IProjectService
    {
        ...
        async Task<IEnumerable<ProjectToListDto>> IProjectService.GetAllProjects()
        {
            var dbProjects = await context.Projects.ToListAsync();
            return dbProjects.Select(x => mapper.Map<ProjectToListDto>(x)).ToList();
        }

        async Task<IEnumerable<ProjectToListDto>> IProjectService.GetUserProjects(ID<UserDto> userID)
        {
        ...
        public async Task<ProjectToListDto> Add(ProjectToCreateDto projectToCreate)
        {
            ...
            project.Title = projectToCreate.Title;
            await context.Projects.AddAsync(project);
            await context.SaveChangesAsync();

            var projectID = new ID<ProjectDto>(project.ID);
            synchronizator.Update(TableRevision.Projects, project.ID);
            return mapper.Map<ProjectToListDto>(projectID);
        }
        ...
        private async Task<bool> UnlinkItem(ID<ItemDto> itemID, ID<ProjectDto> projectID)
        {
            ...
            synchronizator.Update(TableRevision.Projects, (int)projectID);
            return true;
        }
    }
}
