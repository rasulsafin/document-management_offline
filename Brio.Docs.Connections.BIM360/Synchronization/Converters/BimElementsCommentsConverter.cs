...
{
    internal class BimElementsCommentsConverter : IConverter<CommentCreatingData, IEnumerable<Comment>>
    {
        ...
        public Task<IEnumerable<Comment>> Convert(CommentCreatingData commentCreatingData)
        {
            var info = commentCreatingData.IsPreviousDataEmpty
                ? Comments.BimElementsAddedInfo
                : Comments.BimElementsChangedInfo;

            var result = metaCommentHelper.CreateComments(commentCreatingData.Data, MrsConstants.BIM_ELEMENTS_META_COMMENT_TAG, info);
